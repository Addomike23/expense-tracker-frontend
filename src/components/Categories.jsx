...
import { getCategories, createBudget, updateBudget, deleteBudget, getBudgetStatus } from "../services/api";
import { 
  PencilIcon, 
  CheckIcon, 
  XMarkIcon, 
...
function Categories() {
  ...
  const fetchCategories = async () => {
    ...
    try {
      ...
      // Backend: GET /categories?period=month → { success, data: [...], summary }
          ...
          <div className="flex items-center gap-3 w-full sm:w-auto">
            ...
            <div className="relative">
              ...
              <select
                ...
                className="pl-9 pr-8 py-2.5 bg-gray-800/50 border border-gray-700 rounded-xl text-white text-sm focus:outline-none focus:border-yellow-500/50 appearance-none cursor-pointer"
              >
            ...
            {/* Create Button */}
            <button
              onClick={() => { setShowCreateForm(!showCreateForm); setMessage({ type: '', text: '' }); }}
              className="flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-yellow-500 to-orange-500 text-white rounded-xl font-semibold hover:from-yellow-400 hover:to-orange-400 transition-all shadow-lg shadow-yellow-500/25 whitespace-nowrap"
            >
      ...
      {showCreateForm && (
        <div className="bg-gray-800/50 backdrop-blur-xl rounded-2xl border border-gray-700/50 p-6 mb-8 animate-slideDown">
          <div className="flex items-center justify-between mb-5">
            ...
            <button onClick={() => setShowCreateForm(false)} className="text-gray-400 hover:text-white p-1.5 rounded-lg hover:bg-gray-700">
          ...
          <form onSubmit={handleCreateBudget}>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
              <div>
                ...
                <select
                  ...
                  className="w-full px-4 py-3 bg-gray-900/50 border border-gray-700 rounded-xl text-white focus:outline-none focus:border-yellow-500/50"
                >
                  {predefinedCategories.map(cat => (
                    <option key={cat.name} value={cat.name}>{cat.icon} {cat.display}</option>
                  ))}
                </select>
              </div>
              <div>
                ...
                <input
                  ...
                  placeholder={predefinedCategories.find(c => c.name === newBudget.category)?.display}
                  className="w-full px-4 py-3 bg-gray-900/50 border border-gray-700 rounded-xl text-white placeholder-gray-500 focus:outline-none focus:border-yellow-500/50"
                />
              </div>
              <div>
            ...
            <div className="flex gap-3">
              <button type="submit" disabled={creating} className="flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-yellow-500 to-orange-500 text-white rounded-xl font-semibold hover:from-yellow-400 hover:to-orange-400 disabled:opacity-60 transition-all shadow-lg shadow-yellow-500/25">
              ...
              <div key={cat.name || cat._id} className="bg-gray-800/50 backdrop-blur-xl rounded-2xl p-6 border border-gray-700/50 hover:border-gray-600 transition-all group">
                        ...
                        <div className="flex items-center gap-2 mt-2">
                          <div className="relative">
                            ...
                            <input
                              ...
                              className="w-24 pl-7 pr-2 py-1.5 bg-gray-900/50 border border-gray-600 rounded-lg text-white text-sm focus:outline-none focus:border-yellow-500/50"
                              autoFocus
                            />
                          </div>
                          <button onClick={() => handleSave(cat.name)} className="text-emerald-400 hover:text-emerald-300 p-1" title="Save"><CheckIcon className="h-4 w-4" /></button>
                          <button onClick={() => setEditingId(null)} className="text-red-400 hover:text-red-300 p-1" title="Cancel"><XMarkIcon className="h-4 w-4" /></button>
                        </div>
                      ) : (
                        <p className="text-gray-400 text-sm mt-1">Budget: <span className="text-white font-medium">${budget.toFixed(2)}</span></p>
                      )}
                    </div>
                  </div>
                  <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button onClick={() => handleEdit(cat)} className="text-gray-400 hover:text-yellow-400 p-1.5 rounded-lg hover:bg-gray-700" title="Edit"><PencilIcon className="h-4 w-4" /></button>
                    {cat.budget?.id && (
                      <button onClick={() => handleDelete(cat.budget.id)} className="text-gray-400 hover:text-red-400 p-1.5 rounded-lg hover:bg-gray-700" title="Delete"><TrashIcon className="h-4 w-4" /></button>
                    )}
                  </div>
                </div>
                ...
                {status && (
                  <div className={`mt-4 px-3 py-1.5 rounded-lg text-xs font-semibold inline-flex items-center gap-1.5 ${
                    ...
                    {status === 'exceeded' ? <ArrowTrendingUpIcon className="h-3.5 w-3.5" /> :
                     status === 'warning' ? <ExclamationTriangleIcon className="h-3.5 w-3.5" /> :
                     <CheckIcon className="h-3.5 w-3.5" />}
                    {status === 'exceeded' ? 'Over Budget' : status === 'warning' ? 'Approaching Limit' : 'On Track'}
                  </div>
                )}
                {!cat.budget && (
                  <button onClick={() => handleEdit(cat)} className="mt-4 w-full py-2 text-sm text-gray-400 hover:text-yellow-400 border border-dashed border-gray-700 hover:border-yellow-500/50 rounded-xl transition-all">
                    + Set Budget
                  </button>
                )}
              </div>
      ...
      {!error && !loading && categories.length === 0 && (
        <div className="bg-gray-800/50 backdrop-blur-xl rounded-2xl border border-gray-700/50 p-16 text-center">
          ...
          <p className="text-gray-400 mb-8 max-w-md mx-auto">
            Add transactions to see your spending categories, or create a budget to start tracking.
          </p>
          <button
