  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.data?.success) {
        ...
        // Call parent handler — App.jsx will redirect automatically
        if (onLogin) {
          onLogin(response.data.access_token, response.data.user);
        }
        // NOTE: No navigate() here — App.jsx handles redirect via token state
      } else {
    ...
    // No finally block — loading stays true on success until redirect
  };
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#0F172A] py-12 px-4 sm:px-6 lg:px-8 relative overflow-hidden">
      ...
      <div className="max-w-md w-full relative z-10">
          ...
          <div className="p-8">
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div>
                ...
                <div className="relative group">
                  <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                    <UserIcon className={`h-5 w-5 transition-colors duration-300 ${focusedField === 'username' ? 'text-yellow-400' : 'text-gray-500'}`} />
                  </div>
                  <input
                    type="text" required
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <button type="button" className="text-xs text-yellow-400 hover:text-yellow-300 transition-colors">Forgot password?</button>
                </div>
                <div className="relative group">
                  <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                    <LockClosedIcon className={`h-5 w-5 transition-colors duration-300 ${focusedField === 'password' ? 'text-yellow-400' : 'text-gray-500'}`} />
                  </div>
                  <input
              ...
              <div className="flex items-center">
                ...
                <label htmlFor="remember-me" className="ml-2 block text-sm text-gray-400 cursor-pointer select-none">Remember me</label>
              </div>
              ...
              >
                {loading ? (
                  ...
                  <><KeyIcon className="h-5 w-5" /><span>Sign in</span><ArrowRightIcon className="h-5 w-5 group-hover:translate-x-1 transition-transform" /></>
                )}
              </button>

              <p className="text-xs text-gray-500 text-center">Press Enter to submit</p>
            </form>
            ...
            <div className="mt-8">
              <div className="relative">
                ...
                <div className="relative flex justify-center text-sm"><span className="px-4 bg-gray-800 text-gray-500">New to AI Expense Tracker?</span></div>
              </div>
            </div>

            {/* Register Link */}
            <div className="mt-6 text-center">
              <Link to="/register" className="inline-flex items-center gap-2 text-gray-300 hover:text-yellow-400 font-medium transition-all duration-300 group">
        ...
        <div className="mt-8 text-center"><p className="text-xs text-gray-600">Secure login • Your data is encrypted</p></div>
      </div>
