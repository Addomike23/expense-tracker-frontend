  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard name="Total Balance" value={`$${balance.toFixed(2)}`} icon={BanknotesIcon} color="yellow" trend={5.2} />
        <StatCard name="Monthly Income" value={`$${totalIncome.toFixed(2)}`} icon={ArrowTrendingUpIcon} color="emerald" trend={3.8} />
        <StatCard name="Monthly Expenses" value={`$${totalExpenses.toFixed(2)}`} icon={ArrowTrendingDownIcon} color="red" trend={-2.1} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Category Breakdown - Pie/Bar/Area */}
          ...
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="flex bg-gray-700/50 rounded-lg p-1">
              {['pie', 'bar', 'area'].map(view => (
                <button key={view} onClick={() => setChartView(view)} className={`px-3 py-1.5 rounded-md text-xs font-medium transition-all capitalize ${
                  chartView === view ? 'bg-yellow-500/20 text-yellow-400' : 'text-gray-400 hover:text-white'
                }`}>{view}</button>
              ))}
            </div>
          </div>
            ...
            <ResponsiveContainer width="100%" height={300}>
              {chartView === 'pie' ? (
                <PieChart>
                  ...
                  <Tooltip contentStyle={{ background: '#1F2937', border: '1px solid #374151', borderRadius: '12px', color: '#F9FAFB' }} formatter={(v) => `$${v.toFixed(2)}`} />
            ...
            <div className="grid grid-cols-7 gap-1">
              {days.map((day, i) => (
                <div key={i} className="aspect-square flex items-center justify-center">
                  {day && (
                    <div className={`text-sm w-8 h-8 flex items-center justify-center rounded-full transition-all ${
                      day === currentTime.getDate() ? 'bg-yellow-500 text-white font-bold shadow-lg shadow-yellow-500/25' : 'text-gray-300 hover:bg-gray-700'
                    }`}>{day}</div>
                  )}
                </div>
              ))}
            ...
            <div className="mt-4 pt-4 border-t border-gray-700/50 space-y-2">
              ...
              {nextMonthPrediction.prediction && (
                <div className="flex justify-between text-sm"><span className="text-gray-400">Next Month</span>
                  <span className={nextMonthPrediction.trend === 'increasing' ? 'text-red-400 font-medium' : 'text-emerald-400 font-medium'}>${nextMonthPrediction.prediction.toFixed(2)}</span>
                </div>
              )}
            </div>
          </div>
      ...
      <div className="bg-gray-800/30 backdrop-blur-xl rounded-2xl border border-gray-700/50 p-6">
        <div className="flex justify-between items-center mb-6">
          <div>
            ...
            <p className="text-gray-500 text-sm">Your latest activity</p>
          </div>
        ...
        {recentTransactions.length > 0 ? (
                ...
                {recentTransactions.map((t) => (
                  <tr key={t._id || t.id} className="hover:bg-white/5 transition-colors">
                    ...
                    <td className="py-3.5 px-4">
                      <span className="px-2.5 py-1 bg-yellow-500/10 text-yellow-400 rounded-full text-xs font-medium border border-yellow-500/20 capitalize">{t.category}</span>
                    </td>
          ...
          <div className="text-center py-12">
            ...
            <Link to="/add" className="inline-flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-yellow-500 to-orange-500 text-white rounded-xl font-semibold hover:from-yellow-400 hover:to-orange-400 transition-all shadow-lg shadow-yellow-500/25">
